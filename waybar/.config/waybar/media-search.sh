...
play() {
    ...
    kill_current
    local clients='youtube:player_client=tv_simply,android,web_safari'
    local flags=(
        --no-video --audio-display=no
        --ytdl-format='bestaudio/best'
        --ytdl-raw-options="extractor-args=%${#clients}%${clients}"
        --input-ipc-server="$MPV_SOCKET" --really-quiet
    )
    case "$mode" in
